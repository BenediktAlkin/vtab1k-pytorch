...
def vision_mamba(input_shape, patch_size, dim, depth, drop_path_rate=0., **kwargs):
    c, h, w = input_shape
    assert h == w
    assert c == 3
    model = VisionMamba(
...
class VimModel(nn.Module):
    def __init__(self, identifier, input_shape, num_outputs):
        ...
        sd = torch.load(Path(identifier).expanduser().as_posix(), map_location="cpu")["model"]
        sd["head.weight"] = self.model.head.weight.clone()
        sd["head.bias"] = self.model.head.bias.clone()
